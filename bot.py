...
from episode import episode_names, parso_data
...
def r(message):
    if message.chat.type == "private":
        if message.text == "Персонажи":
            ...
            markup = button(episode_names())
            bot.send_message(message.chat.id, 'Выберите эпизот', reply_markup=markup)
        elif message.text in episode_names():
            bot.send_message(message.chat.id, parso_data(message.text))
